	...
	return {
		...
		notify:
			function(m,c,dur,good){
				...
				b.style.cssText+='background-image:url(./images/icons_oth/' + icon + '); background-color:' + bgcolor + ';';
...
//				b.style.top=wh-bh+'px';// this is to place it to the bottom
				b.style.top=(wh / 2) + 'px';// this is to place it to the bottom
//				b.style.left=Math.random()*(ww-bw)+'px';// random x position
//				b.style.left=ww-bw+'px';// this is to place it to the right
				b.style.left=((ww - bw) / 2) + 'px';// this is to place it to the right

				b.style.display='block';
				// fadeout block if supported
				setFading(b,100,0, /* duree ms */ dur,function(){d.body.removeChild(b);});
			},
...
// 				}
				...
				this.main.alertajax=function(m, duree, good){
					Notifier.notify(m,s,duree,good)
				}
			},
