...
class ecampaign{
	...
	/**
	 * Envoi de courriers électroniques E-mails
	 * @param $route Identifiant de la route à utiliser
	 * @param $recipients Liste des destinataires : array( array(mail), array(mail)... )
	 * @param $subject Sujet du mail (optionnel)
	 * @param $message Message du mail (optionnel)
	 ...
	 */
	public function sendMail($route, $recipients, $subject=null, $message=null, $template_id=null, $send_date=null, $group_id=null, $group_fields=null, $group_filter=false, $group_filter_global_op=null, $group_filter_criterias=null){
		$req  = "<sendMail>\n";
		...
		if($reply_to !== null)			$req .= "<replyTo>{$reply_to}</sendDate>\n";
		$req .= "<subject>{$subject}</subject>\n";
		$req .= "<message>{$message}</message>\n";
		$req .= $this->Gen_recip_XML('mail', $recipients, $group_id, $group_fields, $group_filter, $group_filter_global_op, $group_filter_criterias);
		$req .= "</sendMail>";

		// Mise en forme et envoi de la requête
		$xml = "xml=" . $this->makeXML($req);
		$url = $this->url_base . "/sendMail";
		...
		$recipients	= $x->recipients->mail;

		if($code != 0)		throw new Exception($message, $code);

		foreach($recipients as $item){
			$mail_code = (string) $item->attributes()->code;
			$mail_numb = (string) $item->attributes()->number;

			$result[$mail_code . $mail_numb] = ($item->state == "OK"?$item->msgid:false);
		}
